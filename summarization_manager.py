...
def parse_args():
    ...
    parser.add_argument('-prompt_path', '-p', required=True, type=Path, help='Path to the prompt file')
    parser.add_argument('-output_dir', '-o', required=True, type=Path, help='Directory to save the summaries')
    parser.add_argument('-model', required=False, type=str, default='qwen2.5:0.5b', help='Model to use for summarization')
    parser.add_argument('-mode', required=False, type=str, default="update", help='Run mode: update (only run for new offers) or overwrite (run for all offers)')
    parser.add_argument('-verbosity', '-v', required=False, type=int, default=False, help='Verbose mode: 0 - silent, 1 - print system messages, 2 - print system messages and model outputs')
...
def main():
    ...
    for id, offer_details in offers_list:
        ...
        if args.mode == 'update' and output_path.exists():
